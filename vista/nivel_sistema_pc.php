...
<div class="panel-heading">
				...
				<a title="Agregar nuevo registro" class="btn btn-success btn-sm" href="?vista=nivel_sistema_pc&evento=nuevo">Agregar Niveles de conocimientos PC</a>
				...
				<a class="btn btn-default btn-sm" href="?vista=nivel_sistema_pc"  style="float:right; <?php if($_GET['evento']!='nuevo' and $_POST['evento']!='consultar') echo 'display:none;';?>">
...
if($mostrar_formulario==true){
	echo '<form method="post" action="?vista=nivel_sistema_pc">';
	formulario($row_nivel_sistema_pc['cod_nivel_sistema_pc'],$row_nivel_sistema_pc['nombre'],$ultimo_id);
...
function listar($nivel_sistema_pc){
	$salida.='
	<script>
	$(function() {
	...
	"scrollX": true
		...
		<table id="data_table" class="table table-striped">
			...
			';
			$i=0;
	while($row=$nivel_sistema_pc->row()){
	$i++;
	$salida.='
	<tr>
	<td class="td_botones">
		...
		<form method="post"  class="div_botones_listar" style=" margin:0px; display:inline-block; width:105px"> <span style=" float:left; margin-right:1px;">'.$i.' </span>
				...
				<input type="hidden" name="cod_nivel_sistema_pc" value="'.$row['cod_nivel_sistema_pc'].'">
		</form>
	</td>
	<td>'.$row['nombre'].'</td>
	</tr>';
...
function formulario($cod_nivel_sistema_pc,$nombre,$ultimo_id){
echo '
<script type="text/javascript" src="js/js_nivel_sistema_pc.js" ></script>


<input readonly id="cod_nivel_sistema_pc" class="form-control" type="hidden" name="cod_nivel_sistema_pc" value="'.$cod_nivel_sistema_pc.$ultimo_id.'" />
	<div class="row">
